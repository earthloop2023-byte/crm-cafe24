...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/healthz' || url.pathname === '/healthz') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'crm-cafe24-server', db: 'disabled', startedAt, uptimeSec: Math.round(process.uptime()) }));
    return;
  }

  if (url.pathname === '/api/readyz' || url.pathname === '/readyz') {
    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' });
    res.end(JSON.stringify({ ok: true, service: 'crm-cafe24-server', db: 'disabled', ready: true, startedAt, uptimeSec: Math.round(process.uptime()) }));
    return;
  }
...
server.listen(port, '0.0.0.0', () => {
  console.log(`crm-cafe24 health server listening on ${port}`);
});
