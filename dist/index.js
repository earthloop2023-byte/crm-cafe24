...
async function convertCustomerToCompany(customerId) {
...
var PAYMENT_METHOD_BEFORE_DEPOSIT = "\uC785\uAE08 \uC804";
var PAYMENT_METHOD_REFUND_REQUEST = "\uD658\uBD88\uC694\uCCAD";
var PAYMENT_METHOD_DEPOSIT_CONFIRMED = "\uC785\uAE08\uD655\uC778";
...
function normalizeContractPaymentMethod(value) {
  ...
  if (normalized === PAYMENT_METHOD_DEPOSIT_CONFIRMED || normalized === "\uC785\uAE08\uC644\uB8CC" || normalized === "\uAD6D\uBBFC\uC740\uD589" || normalized === "\uCE74\uB4DC\uACB0\uC81C" || normalized === "\uD06C\uBABD" || ["deposit", "deposited", "banktransfer", "transfer", "confirmed", "kb", "kookmin", "kbstar", "card", "cardpayment", "kmong"].includes(asciiKey)) {
...
async function registerRoutes(httpServer2, app2) {
  ...
  app2.get("/api/contracts/paged", async (req, res) => {
    try {
      ...
      const result = await storage.getContractsPaged({
        ...
        managerName: isManagerPosition(currentUser?.role) ? currentUser?.name || void 0 : managerName && managerName !== "all" ? managerName : void 0,
        customerName: customerName && customerName !== "all" ? customerName : void 0,
        productCategory: productCategory && productCategory !== "all" ? productCategory : void 0,
        paymentMethod: paymentMethod && paymentMethod !== "all" ? paymentMethod : void 0,
  ...
  app2.get("/api/contracts", async (req, res) => {
    try {
      ...
      const filteredContracts = isManagerPosition(currentUser?.role) ? contracts2.filter((contract) => isOwnManagedRecord(currentUser, contract)) : contracts2;
      res.json(filteredContracts.map((contract) => sanitizeFinancialContractRow(contract, currentUser?.role)));
    } catch (error) {
  ...
  app2.get("/api/contracts-with-financials", async (req, res) => {
    try {
      ...
      const filteredData = isManagerPosition(currentUser?.role) ? data.filter((contract) => isOwnManagedRecord(currentUser, contract)) : data;
      res.json(filteredData.map((contract) => sanitizeFinancialContractRow(contract, currentUser?.role)));
    } catch (error) {
...
function listenOnPort(server, listenPort, label = "primary") {
